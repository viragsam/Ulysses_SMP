...
| `shadow_heightmap.py` | Horizon-shadowing over a synthetic crater heightmap: per-cell, per-frame line-of-sight to the Sun. Also writes an illumination map (% of the CSV's timespan each cell is lit). `python shadow_heightmap.py [csv] [gif]`. |
...
shadow map over every row in the CSV into an illumination map: on the
...
still sees ~55%. That's the more interesting illumination map of the two.
